...
protected:
	...
	void callStringAddress(std::string const& _name, std::string const& _arg1, util::h176 const& _arg2)
	{
		BOOST_CHECK(call(_name + "(string,address)", u256(0x40), _arg2, _arg1.length(), _arg1).empty());
	}

	void callStringAddressBool(std::string const& _name, std::string const& _arg1, util::h176 const& _arg2, bool _arg3)
	{
	...
	util::h176 callStringReturnsAddress(std::string const& _name, std::string const& _arg)
	{
		bytes const& ret = call(_name + "(string)", u256(0x20), _arg.length(), _arg);
		BOOST_REQUIRE(ret.size() == 0x20);
		BOOST_CHECK(std::count(ret.begin(), ret.begin() + 12, 0) == 12);
		bytes const addr{ret.begin() + 12, ret.end()};
		return util::h176(addr);
	}

	std::string callAddressReturnsString(std::string const& _name, util::h176 const& _arg)
	{
