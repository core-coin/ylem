...
protected:
	void deployWallet(
		u256 const& _value = 0,
		vector<h176> const& _owners = vector<h176>{},
		u256 _required = 1,
		u256 _dailyLimit = 0
	)
	{
...
BOOST_AUTO_TEST_CASE(add_owners)
{
	...
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x13)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x13)) == encodeArgs(true));
	// and check that a non-owner cannot add a new owner
	m_sender = account(0);
	sendEther(account(2), 10 * ether);
	m_sender = account(2);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x20)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x20)) == encodeArgs(false));
...
{
	deployWallet(200);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x12)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x12)) == encodeArgs(true));
	BOOST_REQUIRE(callContractFunction("changeOwner(address,address)", h176(0x12), h176(0x13)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x12)) == encodeArgs(false));
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x13)) == encodeArgs(true));
}
...
BOOST_AUTO_TEST_CASE(remove_owner)
{
	...
	{
		BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x12 + i)) == encodeArgs());
		BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x12 + i)) == encodeArgs(true));
	}
	...
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(callContractFunction("removeOwner(address)", h176(0x12 + i)) == encodeArgs());
	// check the result
	for (unsigned i = 0; i < 10; ++i)
		BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x12 + i)) == encodeArgs(i % 2 == 0));
	// add them again
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x12 + i)) == encodeArgs());
...
{
	vector<h176> owners{
		h176("0x42c56279432962a17176998a4747d1b4d6ed4367"),
		h176("0xd4d4669f5ba9f4c27d38ef02a358c339b5560c47"),
		h176("0xe6716f9544a56c530d868e4bfbacb172315bdead"),
		h176("0x775e18be7a50a0abb8a4e82b1bd697d79f31fe04"),
		h176("0xf4dd5c3794f1fd0cdc0327a83aa472609c806e99"),
		h176("0x4c9113886af165b2de069d6e99430647e94a9fff"),
		h176("0x3fb1cd2cd96c6d5c0b5eb3322d807b34482481d4")
	};
...
BOOST_AUTO_TEST_CASE(multisig_value_transfer)
{
	...
	h176 destination = h176("0x5c6d6026d3fb35cd7175fd0054ae8df50d8f8b41");
...
BOOST_AUTO_TEST_CASE(revoke_addOwner)
{
	...
	BOOST_REQUIRE(callContractFunction("changeRequirement(uint256)", u256(3)) == encodeArgs());
	// add a new owner
	h176 deployer = m_sender;
	h256 opHash = util::keccak256(FixedHash<4>(util::keccak256("addOwner(address)")).asBytes() + h256(0x33).asBytes());
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x33)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x33)) == encodeArgs(false));
	m_sender = account(0);
	sendEther(account(1), 10 * ether);
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x33)) == encodeArgs());
	...
	m_sender = account(3);
	BOOST_REQUIRE(callContractFunction("addOwner(address)", h176(0x33)) == encodeArgs());
	BOOST_REQUIRE(callContractFunction("isOwner(address)", h176(0x33)) == encodeArgs(true));
}
