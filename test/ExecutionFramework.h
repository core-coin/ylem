...
public:
	...
	virtual bytes const& compileAndRunWithoutCheck(
		...
		std::map<std::string, util::h176> const& _libraryAddresses = {}
	) = 0;
	...
	static bytes encode(util::h176 const& _value) { return encode(util::h256(_value, util::h256::AlignRight)); }
...
protected:
	...
	void sendEther(util::h176 const& _to, u256 const& _value);
	size_t currentTimestamp();
	size_t blockTimestamp(u256 _number);

	/// @returns the (potentially newly created) _ith address.
	util::h176 account(size_t _i);

	u256 balanceAt(util::h176 const& _addr);
	bool storageEmpty(util::h176 const& _addr);
	bool addressHasCode(util::h176 const& _addr);

	size_t numLogs() const;
	size_t numLogTopics(size_t _logIdx) const;
	util::h256 logTopic(size_t _logIdx, size_t _topicIdx) const;
	util::h176 logAddress(size_t _logIdx) const;
	...
	util::h176 m_sender = account(0);
	util::h176 m_contractAddress;
